...
class DeepResearcher:
    ...
    def __init__(self, client, model: str, max_iterations: int = 5, max_sources: int = 30, logger = None):
        self.client = client
        self.model = model
        self.logger = logger
        ...
        self.llm_calls = []
        self.total_llm_latency_seconds = 0
    ...
    def _make_llm_call(self, system_prompt: str, user_prompt: str, temperature: float = 0.7, max_tokens: int = 1000, call_name="LLM CALL") -> Any:
        """
        Make an LLM API call and track metrics (tokens and latency).
        """
        start_time = datetime.now()
        if self.logger:
            self.logger.info(f"\n{'='*80}\nLLM Call #{len(self.llm_calls) + 1}: {call_name}\n{'='*80}")
            self.logger.info(f"\n--- SYSTEM PROMPT ---\n{system_prompt}\n")
            self.logger.info(f"\n--- USER PROMPT ---\n{user_prompt}\n")
            self.logger.info(f"\n--- PARAMETERS ---")
        ...
        # CRITICAL FIX: Ensure content is a string, not tuple or other type
        # Handle case where prompt might be wrapped in tuple
        if isinstance(system_prompt, (tuple, list)):
            assert len(system_prompt) == 1, "issue"
        ...
        # Always use plain string format
        messages = [
        ...
        try:
            ...
            error_str = str(e)
            self.logger.info(f"❌ LLM call failed: {error_str[:300]}")
            self.logger.info(f"   System prompt type: {type(system_prompt)}")
            self.logger.info(f"   User prompt type: {type(user_prompt)}")
        ...
        if self.logger:
            ...
            self.logger.info(f"Latency: {latency_seconds:.3f}s\n")
    ...
    def fix_incomplete_report(self, report: str, validation: Dict[str, Any], original_query: str) -> str:
        ...
        """
        self.logger.info("🔧 Attempting to fix incomplete report...")
        ...
        if "Research Questions for Investigation" in fixed_report:
            ...
            self.logger.info("   - Removed incomplete research questions section")
...
"""
            ...
            self.logger.info("   - Added completion note due to short report length")
        ...
        if new_validation["is_complete"]:
            self.logger.info("✅ Report successfully fixed and validated")
        else:
            self.logger.info(f"⚠️  Report still has {len(new_validation['issues'])} issues after fixing")
    ...
    def decompose_query(self, system_prompt: str, initial_query: str) -> List[str]:
        ...
        If the initial_query is in english, then genenrate the sub-query in english. if the initial query is in chinese, then generate the sub-query in chinese
        """
    ...
    def perform_web_search(self, queries: List[str]) -> str:
        ...
        #     self.logger.info(f"⚠️  Warning: session_manager not available in perform_web_search (session_id: {getattr(self, 'session_id', 'N/A')})")
        #     self.session_manager = None
        # else:
        #     self.logger.info(f"📊 Using existing session manager for web search (session_id: {self.session_id}, manager: {id(self.session_manager)})")
        ...
        for i, query in enumerate(queries):
            try:
                ...
                enhanced_query, _ = web_search_run("", search_query, None, None, {
                    ...
                    "delay_seconds": None,  # Use default random delay (4-32 seconds)
                    "headless": False,  # Allow CAPTCHA solving if needed
                    "session_manager": None  # Use shared browser session
                })
                
                
                self.logger.info(f"[enhanced_query]: {enhanced_query}")
    ...
    def extract_and_fetch_urls(self, search_results: str) -> Tuple[str, List[Dict]]:
        ...
        try:
            ...
            content_with_urls = search_results 
    ...
    def evaluate_completeness(self, system_prompt: str, query: str, current_synthesis: str) -> Tuple[bool, List[str]]:
        ...
        try:
            response = self._make_llm_call(
                ...
                temperature=0.3,
                max_tokens=500,
                call_name="evaluate_completeness"
            )
            
            content = response.choices[0].message.content.strip()
            # Clean reasoning tags from completeness evaluation response
            content = clean_reasoning_tags(content)
            # self.logger.info("DEBUG - EVAL COMPLETENESS", response.usage)
    ...
    def generate_preliminary_draft_with_initial_search(self, system_prompt: str, retrieval_content: str, initial_query: str) -> str:
        """
        ...
        draft_prompt = f"""
        Generate a preliminary research report structure for the following query using your internal knowledge and initial retrieved information as background information
        ...
        If the initial_query is in english, then genenrate english report. if the initial query is in chinese, then generate the report in chinese
        ...
        self.logger.info("================================ draft prompt =============================")
        # self.logger.info(draft_prompt)
        self.logger.info(f"len of draft_prompt: {len(draft_prompt)}")
        self.logger.info("===========================================================================")
        ...
        try:
            response = self._make_llm_call(
                ...
                temperature=0.7,
                max_tokens=50000,
                call_name="generate_preliminary_draft"
            )
            
            draft = response.choices[0].message.content.strip()
            draft = clean_reasoning_tags(draft)
            # self.logger.info("DEBUG - GENERATE PRELIM DRAFT WITH INIT SEARCH", response.usage)
    ...
    def generate_preliminary_draft(self, system_prompt: str, initial_query: str) -> str:
        ...
        try:
            ...
            draft = response.choices[0].message.content.strip()
            draft = clean_reasoning_tags(draft)
            # self.logger.info("DEBUG - GENERATE PRELIM DRAFT", response.usage)
    ...
    def analyze_draft_gaps(self, current_draft: str, original_query: str) -> List[Dict[str, str]]:
        ...
        try:
            response = self._make_llm_call(
                ...
                user_prompt=gap_analysis_prompt,
                temperature=0.3,
                max_tokens=1000,
                call_name="analyze_draft_gaps"
            )
            
            content = response.choices[0].message.content.strip()
            content = clean_reasoning_tags(content)
            # self.logger.info("DEBUG - ANAL DRAFT GAPS", response.usage)
    ...
    def perform_gap_targeted_search(self, gaps: List[Dict[str, str]]) -> str:
        ...
        # if not hasattr(self, 'session_manager') or self.session_manager is None:
        #     # Log warning - this shouldn't happen in normal flow
        #     self.logger.info("⚠️  Warning: session_manager not available in perform_web_search")
        #     self.session_manager = None
    ...
    def denoise_draft_with_retrieval(self, current_draft: str, retrieval_content: str, original_query: str) -> str:
        ...
        self.logger.info("================================= denoise prompt ==============================")
        # self.logger.info(denoising_prompt)
        self.logger.info(f"len(denoising_prompt): {len(denoising_prompt)}")
        self.logger.info(f"len(current_draft): {len(current_draft)}")
        self.logger.info(f"len(retrieval_content): {len(retrieval_content)}")
        self.logger.info("================================================================================")

        
        try:
            response = self._make_llm_call(
                system_prompt="You are an expert research synthesizer performing draft denoising.",
                user_prompt=denoising_prompt,
                temperature=0.6,
                max_tokens=20000,
                call_name="denoise_draft_with_retrieval"
            )
            
            denoised_draft = response.choices[0].message.content.strip()
            denoised_draft = clean_reasoning_tags(denoised_draft)
            # self.logger.info("DEBUG - DENOISE", response.usage)
    ...
    def evaluate_draft_quality(self, draft: str, previous_draft: str, original_query: str) -> Dict[str, float]:
        ...
        try:
            response = self._make_llm_call(
                ...
                temperature=0.2,
                max_tokens=500,
                call_name="evaluate_draft_quality"
            )
            
            content = response.choices[0].message.content.strip()
            content = clean_reasoning_tags(content)
            # self.logger.info("DEBUG - EVAL DRAFT", response.usage)
    ...
    def research(self, system_prompt: str, initial_query: str) -> Tuple[str, int]:
        ...
        #         {
        #             "metrics": {
        #                 "total_time_seconds": round(100.2, 3),
        #                 "llm_calls": 13,
        #                 "total_tokens": 1,
        #                 "completion_tokens": 1,
        #                 "prompt_tokens": 1,
        #                 "total_llm_latency_seconds": 100,
        #                 "start_time": "today",
        #                 "end_time": "today",
        #             },
        ...
        #     self.logger.info(f"🔬 Starting deep research with session ID: {self.session_id} (DeepResearcher instance: {id(self)})")
        # else:
        #     self.logger.info("⚠️ Failed to create browser session, proceeding without web search")
            
        try:
            self.current_draft = None
            # PHASE 1: INITIAL RESEARCH - Ensure we always gather external sources
            self.logger.info("TTD-DR: Performing initial research...")

            initial_queries = self.decompose_query(system_prompt, initial_query)

            if initial_queries:
                self.logger.info(f"  - Searching for {len(initial_queries)} initial topics...")
                initial_search_results = self.perform_web_search(initial_queries)
                self.logger.info(f"[initial_search_results] {initial_search_results}")
                # Extract and fetch URLs from initial search
                if initial_search_results and "Web Search Results" in initial_search_results:
                    self.logger.info("  - Extracting initial sources...")
                    ...
                    self.logger.info(f"  - Found {len(initial_sources)} initial sources")

                    self.logger.info("TTD-DR: Generating preliminary draft with initial research...")
                    self.current_draft = self.generate_preliminary_draft_with_initial_search(system_prompt, initial_content, initial_query)
                else:
                    self.logger.info("  - No sources found in initial search")
            else:
                self.logger.info("  - Warning: Could not decompose query for initial research")
                ...
                if fallback_search_results and "Web Search Results" in fallback_search_results:
                    ...
                    for source in fallback_sources:
                        if 'url' in source:
                            self.citation_counter += 1
                            self.citations[self.citation_counter] = source
                    self.logger.info(f"  - Fallback search found {len(fallback_sources)} sources")


            # PHASE 1: INITIALIZATION - Generate preliminary draft (updatable skeleton)
            if self.current_draft is None: 
                self.logger.info("TTD-DR: Generating preliminary draft without initial research...")
            ...
            for iteration in range(self.max_iterations):
                ...
                self.logger.info("  - Performing denoising step...")
                ...
                self.logger.info("========================== prev draft =============================")
                self.logger.info(previous_draft)
                self.logger.info("========================== current draft =============================")
                self.logger.info(self.current_draft)
                self.logger.info("===================================================================")

                
                # STEP 5: Evaluate quality improvement
                self.logger.info("  - Evaluating draft quality...")
                ...
                self.logger.info(f"  - Quality scores: Completeness={completeness:.2f}, Improvement={improvement:.2f}")
                ...
                if completeness > 0.9 or (improvement < 0.03 and completeness > 0.7):
                    self.logger.info("  - Quality threshold reached, research complete")
                    break

            # PHASE 3: FINALIZATION - Polish the final draft
            self.logger.info("TTD-DR: Finalizing research report...")
            
            # Ensure we have gathered some sources
            if len(self.citations) == 0:
                self.logger.info("⚠️  Warning: No external sources found during research!")
                self.logger.info("   Deep research should always consult external sources.")
            else:
                self.logger.info(f"✅ Research completed with {len(self.citations)} sources")
            ...
            return (
                final_report, 
                {
                    "metrics": {
                        ...
                        "total_llm_latency_seconds": self.total_llm_latency_seconds,
            ...
            # if self.session_manager:
            #     self.logger.info(f"🏁 Closing research session: {self.session_id}")
        ...
        **CRITICAL QUALITY REQUIREMENTS**:
        ...
        try:
            response = self._make_llm_call(
                ...
                temperature=0.5,
                max_tokens=50000,
                call_name="finalize_research_report"
            )
            ...
            if not validation["is_complete"]:
                self.logger.info(f"⚠️  Report validation found {len(validation['issues'])} issues:")
                ...
                self.logger.info("✅ Report validation passed - report is complete")
            ...
            self.logger.info(f"📊 Citation Statistics:")
            self.logger.info(f"   - Used citations: {citation_validation['citations_used']}/{citation_validation['citations_total']}")
            self.logger.info(f"   - Usage percentage: {citation_validation['usage_percentage']:.1f}%")

            if "warning" in citation_validation:
                self.logger.info(f"⚠️  {citation_validation['warning']}")
                if len(citation_validation['unused_citations']) > 0:
                    self.logger.info(f"   - Unused citations: {citation_validation['unused_citations'][:10]}" +
            ...
            metadata += f"- Completion tokens used: {self.completion_tokens}\n"
            metadata += f"- Prompt tokens used: {self.prompt_tokens}\n"

            # return polished_report + references + metadata
            return polished_report + references
